...
integration_test: show_env restart_snap deps build dist install test_integration test_jupyter_unit
...
test_jupyter_lint:
	docker run -ti intelsdi/swan-jupyter pep8 --max-line-length=120 .

test_jupyter_unit:
	docker run -ti --rm intelsdi/swan-jupyter python test_swan.py
